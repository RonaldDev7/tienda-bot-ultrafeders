...
// Función para obtener configuración por servidor
...
client.once('ready', async () => {
  ...
  client.guilds.cache.forEach(async guild => {
    ...
    if (!cfg) {
      console.log(`⚠️ No hay config para este servidor: ${guild.name}`);
      return;
    }

    try {
      const canal = await client.channels.fetch(cfg.CANAL_COMPRA_ID);

      if (!canal) {
        console.log(`❌ No se encontró canal en ${guild.name}`);
        return;
      }
  ...
  if (interaction.isModalSubmit() && interaction.customId.startsWith('modal_compra_')) {
    ...
    const canal = await interaction.guild.channels.create({
      ...
      permissionOverwrites: [
        ...
        { id: cfg.ROLE_STAFF_ID, allow: ['ViewChannel', 'SendMessages'] }
      ]
    });
