...
// Validación de IDs requeridas
if (!CANAL_CATALOGO_ID || !CANAL_COMPRA_ID) {
  console.log("❌ Faltan CANAL_CATALOGO_ID o CANAL_COMPRA_ID en la configuración del servidor.");
  process.exit(1);
}
...
client.once("ready", async () => {
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      {
        name: '🛒 Cómo comprar',
        // FIX: usar CANAL_COMPRA_ID en lugar de CANAL_ID (que era undefined)
        value: `Dirígete al canal <#${CANAL_COMPRA_ID}> e inicia un ticket. Un administrador se comunicará contigo al instante.`
      },
      {
        name: '📦 Tipos de entrega',
        value: '• Gamepass: Recibirás los Robux en un plazo de hasta 5 días.\n• Grupo: El primer pago tarda aproximadamente 3 días; después las entregas son inmediatas.'
      }
    )
